...
describe('E2E: Dashboard Page incidents gadget test', function () {
    ...
    beforeEach(function () {
        ...
        browser.get('#/dashboard/incidents/');
    ...
    it('#1 testing redirect function to login page', function () {
        ...
        expect(globalPage.dashboard.declineAccessMessage.getText()).toBe(globalPage.incidents.expectedDeclineAccessMessage);
        expect(loginPage.auth.errorMessage.isPresent()).toBe(false);
        expect(loginPage.auth.logoutMessage.isPresent()).toBe(false);
        loginPage.auth.login('admin', 'admin');
        browser.waitForAngular();
        expect(browser.getCurrentUrl()).toContain(globalPage.incidents.expectedCorrectUrl);
