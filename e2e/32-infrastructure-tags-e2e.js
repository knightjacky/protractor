...
describe('E2E: infrastructure Page tags tab test', function () {
    ...
    beforeEach(function () {
        ...
        browser.get('#/infrastructure/tags/');
    ...
    it('#1 testing redirect function to login page', function () {
        ...
        expect(infrastructurePage.elements.declineAccessMessage.getText()).toBe(infrastructurePage.tags.expectedDeclineAccessMessage);
        expect(loginPage.auth.errorMessage.isPresent()).toBe(false);
        expect(loginPage.auth.logoutMessage.isPresent()).toBe(false);
        loginPage.auth.login('admin', 'admin');
        browser.waitForAngular();
        expect(browser.getCurrentUrl()).toContain(infrastructurePage.tags.expectedCorrectUrl); // verify after login infrastructure web page
